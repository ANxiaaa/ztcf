<template>
  <div class="carParam">
    ...
    <van-tabs v-model="active" swipeable :line-width="40 / 75 + 'rem'" :line-height="8 / 75 + 'rem'" color="#4771E6" sticky @change="changeOrder" title-active-color="#4771E6" title-inactive-color="#666">
      <!-- <van-tab title="基本信息" :key="-1">
        <ul class="box t600">
          <li>
            <h5>基本信息</h5>
            <p v-for="(i, index) in field" :key="index" v-if="!i.list && !i.logo">
              <span>{{i[Object.keys(i)]}}</span>
              <span>{{carData[Object.keys(i)] || '-'}}</span>
            </p>
          </li>
        </ul>
      </van-tab> -->
      <van-tab v-for="(i, index) in field" v-if="i.list" :key="index" :title="i[Object.keys(i)[0]]" :name="'a' + index">
        <ul class="box t600">
          <li>
            <h5>{{i[Object.keys(i)[0]]}}</h5>
            <p v-for="(a, idx) in i[Object.keys(i)[1]]" v-if="Object.keys(a)[0] != 'color'" :key="'b' + idx">
              <span>{{a[Object.keys(a)]}}</span>
              <span>{{aaaa(carData[Object.keys(i)[0]],[Object.keys(a)[0]]) || '-'}}</span>
            </p>
          </li>
        </ul>
      </van-tab>
      <van-tab title="车型颜色" key="color">
        <ul class="box t600">
          <li>
            <h5>颜色种类</h5>
            <p v-for="i in carData.body.color.split('|')" :key="i">
              <span>{{i.split(',')[0]}}</span>
...
export default {
  ...
  mounted(){
    this.$store.commit('changeTitle', '详细参数')
    this.$api.carList.findCarParamByCarId(this.id).then(res=>{
      console.log(res)
      if(res.code == 200){
