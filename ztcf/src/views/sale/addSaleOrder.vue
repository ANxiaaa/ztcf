  ...
  <div class="addSaleOrder">
    <div @click="address" class="addMsg">
      ...
      <div class="showadd" v-if="isAdd">
        ...
        <div class="addname">
          <p><b>{{addressE.name}}</b><span>{{addressE.phone}}</span></p>
    ...
    <div class="goodsMsg">
      ...
      <div class="main">
        ...
        <div class="right">
          <div class="text">
            ...
            <p class="price">
              <b>¥{{(Number(orderInfo.product.showCost) / 10000).toFixed(2)}}万</b>
              <s>¥{{(Number(orderInfo.product.price) / 10000).toFixed(2)}}万</s>
            </p>
          </div>
      ...
      <p class="fwxy"><b>服务费</b><b>¥{{saleOrder.orderMoney}}</b></p>
      <div class="line"></div>
      <p class="pay"><b>应支付费用</b><b style="color: #D95A41">¥{{saleOrder.orderPay}}</b></p>
      <p class="field"><b>备注</b><textarea @input="input" v-model="saleOrder.remark" rows="1" placeholder="请在此输入相关要求"></textarea></p>
    </div>
    <div class="line"></div>
...
export default {
  ...
  methods:{
    ...
    address(){
      ...
      this.$router.push(`/addressList?from=saleOrder`)
    },
    // 提交订单
    subOrder(){
      // this.showPay = true
      let data = this.saleOrder
      console.log(data)
  ...
  mounted(){
    ...
    let receiptId = this.saleOrder.logistics.receiptId
    if(receiptId == -1){
      this.isAdd = false
    }else{
      this.$api.user.getReceiptById(receiptId).then(res=>{
        if(res.code == 200){
          this.isAdd = true
  ...
  computed:{
    // 订单信息
    saleOrder(){
      let data = this.$store.getters.saleOrder
